...
void worker(vector<ui> valid_sums, ThreadResult& output)
{
    ui ans = 0;
...
int main()
{
    ...
    {
        vector<ui> jobs;
        for (ui j = i; j < valid_sums.size(); j += n_threads)
            jobs.push_back(valid_sums[j]);
        thread new_thread(worker, jobs, ref(thread_results[i]));
